...
pub fn input_generator(input: &[u8]) -> Vec<u8> {
    ...
    if data.last() != Some(&b'\n') {
        ...
        println!("{}",*data.last().unwrap() as char);
    }
    data
}
...
pub fn part1(input: &[u8]) -> usize {
    ...
    while let Some((x,y)) = queue.pop_front() {
        ...
        if x < (COL_LENGTH-1) { // len vs start-by-0
            neighbours.push((x+1,y));
        }
        if y > 0 {
            neighbours.push((x,y-1));
        }
        if y < (ROWS-1) { // len vs start-by-0
            neighbours.push((x,y+1));
        }
        // dbg!(neighbours);
        ...
        while neighbours.len > 0 {
            ...
            match (height > height_n && height_n != b'E' && height_n != b'S', diff,height,height_n) {
                (true,_,_,_) | (_,0..=1,_,_) | (_,_,b'S',b'a'..=b'b') | (_,_,_,b'S') => {
...
pub fn part2(input: &[u8]) -> usize {
    ...
    while let Some((x,y)) = queue.pop_front() {
        ...
        while neighbours.len > 0 {
            ...
            // println!("Checking ({x},{y}):{}->({xn},{yn}):{}",height as char, height_n as char);
            let diff = height.abs_diff(height_n);
            match (height < height_n && height != b'E' && height_n != b'a', diff,height, height_n) {
                (_,_,b'b',b'a') => {
                    visited[yn * COL_LENGTH + xn] = Some((x,y));
                    //queue.push((xn,yn)); // for path
                    end = (xn,yn);
...
fn print_visited(visited: &[Option<(usize,usize)>], cols: usize, rows: usize, start: (usize,usize), end: (usize,usize)) {
    use std::io::Write;
